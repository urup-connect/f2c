...
from app.authn.models import PasskeyCredential


class RelyingPartyConfigTests(SimpleTestCase):
    """A missing RP ID must stop the ceremony, not produce a broken one."""

    @override_settings(WEBAUTHN_RP_ID='cultivatorscollective.co.za')
    def test_rp_id_is_read_from_settings(self):
        self.assertEqual(wa.rp_id(), 'cultivatorscollective.co.za')
