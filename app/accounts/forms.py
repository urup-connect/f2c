...
nothing about, and both the mobile and nickname constraints carry a condition,
which ``ModelForm`` validation does not reach. So they are checked by hand.
"""
...
class ContactClashMixin(forms.ModelForm):
    """Turns the mobile and nickname constraints into field-level errors.

    Both are enforced by a partial unique index on ``User``; without this the
    ...
    def clean_nickname(self):
        ...
        clash = User.objects.by_nickname(value)
        if self.instance.pk:
            clash = clash.exclude(pk=self.instance.pk)
        if clash.exists():
            raise ValidationError(
                'Another account already holds that nickname.',
                code='nickname_taken',
            )
        return value
