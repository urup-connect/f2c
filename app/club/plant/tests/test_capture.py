...
from app.club.strains.models import ListingStatus
...
class SharedRuleTests(PlantTestCase):
    ...
    def test_another_cultivator_cannot_capture_against_this_listing(self):
        ...
        The caller is appointed to a different farm, so the listing they are
        capturing against is not theirs — which is the check, and it is a join
        against `ProducerMembership` rather than an equality on an account.
        """
        _other_grower, other_farm = self.another_cultivator()

        with self.assertRaises(ValidationError):
            capture_plant(other_farm, **raw())
...
class CommandTests(PlantTestCase):
    ...
    def test_a_name_that_is_not_a_producer_is_refused(self):
        """The command names a **farm** by trading name now.

        It took an email address or a nickname and resolved a person, which had
        three equally good answers for a farm with three appointed staff.
        """
        with self.assertRaises(CommandError) as refused:
            self.add(**{'--cultivator': 'Sam'})
