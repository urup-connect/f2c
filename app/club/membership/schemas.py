...
class MemberRowOut(Schema):
    ...
    def resolve_nickname(obj):
        """From the membership, where it lives since C27."""
        return obj.club_nickname

    @staticmethod
    def resolve_status(obj):
        """The **membership's** standing, which is what this register is about.

        Not the account's. An unpaid member signs in perfectly well, so the
        account status would show every one of them as Active — and the column
        an administrator reads this in exists to tell them apart.
        """
        return obj.club_membership.status

    @staticmethod
    def resolve_status_label(obj):
        return obj.club_membership.get_status_display()

    @staticmethod
    def resolve_role(obj):
        """A word for what this member also is, derived from the relationships.

        There is no role column since C28, and a member may be several things at
        once — so this reports the most capable, exactly as ``UserOut.role``
        does, and for the same reason: a column has to show something.
        """
        ...
        if any(True for _ in obj.producer_appointments.all()):
            return 'cultivator'
        if obj.club_membership.is_sharing_member:
...
class MemberOut(Schema):
    ...
    def resolve_registered_by(obj):
        """The **producer** whose stock a placeholder holds, by trading name.

        It pointed at the person who keyed the placeholder in, and points at the
        farm since the producer generalisation: the stock is the farm's, and the
        record must not be orphaned when that person leaves. Null for everybody
        who is not a placeholder.
        """
        producer = obj.club_membership.registered_by
        return producer.trading_name if producer is not None else None
    ...
    def resolve_editable(obj):
        ...
        return obj.deleted_at is None and not obj.is_sharing_member
