...
from app.core.accounts.models import IdentityNumberDisclosure, UserStatus
...
class TheRegisterEndpoint(MemberApiTestCase):
    ...
    def test_it_lists_club_members_newest_first(self):
        """Members only, and the administrator and the grower are not members.

        `setUp` creates all three, and only one of them joined the club — which
        is the correct register: an administrator runs the club without joining
        it, and a producer's appointed staff never did either.
        """
        response = self.get()

        self.assertEqual(200, response.status_code)
        nicknames = [row['nickname'] for row in response.json()]
        self.assertEqual(['Thabo'], nicknames)
    ...
    def test_a_row_carries_the_status_and_role_labels(self):
        ...
        self.assertEqual('member', row['role'])
    ...
    def test_the_filters_narrow(self):
        self.assertEqual(
            [],
            [row['nickname'] for row in self.get(role='cultivator').json()],
        )
...
class TheRecordEndpoint(MemberApiTestCase):
    ...
    def test_the_disclosure_history_comes_down_with_the_record(self):
        ...
        self.assertEqual(self.admin.display_name, record['disclosures'][0]['read_by'])
        self.assertEqual(
...
class Suspending(MemberApiTestCase):
    ...
    def test_suspending_your_own_membership_is_a_422(self):
        """An administrator who is **also a member** — the case C28 made real.

        The fixture has to hold both relationships now: an administrator who
        never joined the club is not on its register at all, so the endpoint
        404s before this rule is reached. That is correct, and it means the
        rule only ever applied to somebody wearing both hats.
        """
        make_member('registrar@example.com', 'Registrar', account=self.admin)
    ...
    def test_an_administrator_who_never_joined_is_not_on_the_register(self):
        """The 404 the test above works around, asserted as the rule it is."""
        self.assertEqual(
            404, self.post(self.member_path(self.admin, '/suspend')).status_code
        )
...
class DisclosingAnIdentityNumber(MemberApiTestCase):
    ...
    def test_it_answers_the_number_and_the_record_of_the_read(self):
        ...
        self.assertEqual(self.admin.display_name, body['disclosure']['read_by'])
