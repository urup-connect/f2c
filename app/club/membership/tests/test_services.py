...
from app.core.accounts.roles import MEMBER_ACTIONS, permissions_for
from app.core.common import crypto
from app.core.documents import services as document_services
from app.core.documents.models import Agreement, DocumentConsent
...
def permissions_once_active(user):
    """What this account will hold once a payment activates the membership.

    ``permissions_for`` grants nothing for a membership that is not active, and
    a registration deliberately leaves one at ``pending_payment`` — so asking it
    directly answers "nothing" and would prove nothing about *what kind* of
    membership was made. Flipped in memory only; nothing is saved.

    **What is flipped changed with C27.** It used to be the account status,
    because that is where the block was. The account is Active from the moment
    it exists now, and the block is on the membership.
    """
    membership = user.club_membership
    membership.status = MembershipStatus.ACTIVE
    loaded = User.objects.with_platform_roles().get(pk=user.pk)
    loaded.club_membership.status = MembershipStatus.ACTIVE
    return permissions_for(loaded)
...
class RegisterMemberTests(RegistrationTestCase):
    def test_a_registration_lands_at_pending_payment(self):
        ...
        self.assertEqual(result.user.club_membership.status, MembershipStatus.PENDING_PAYMENT)

    def test_a_registration_makes_a_member_and_only_a_member(self):
        """The one relationship sign-up can create.

        Asserted against the membership *and* against what it will carry once
        paid, because the second is the part that matters: a form that could
        hand out cultivation or administrative authority would be a way to
        claim it.
        """
        result = services.register_member(**self.submission())

        # No role column since C28. What a registration makes is a
        # membership, and that is what says they are a member.
        self.assertEqual(
            result.user.club_membership.status,
    ...
    def test_a_registered_member_holds_nothing_until_payment(self):
        """The membership exists from registration; the authority does not.

        `pending_payment` is not active, so it grants nothing — and the account
        is signed in throughout, which is what sends them to the payment screen
        rather than to a refusal at the door. C27.
        """
        result = services.register_member(**self.submission())
    ...
    def test_a_registered_member_signs_in_and_owes_the_club(self):
        """**This asserted the opposite until C27, and the reversal is the point.**

        Registration used to leave the *account* at `pending_payment`, and
        `is_active` is derived from that — so an unpaid member could not sign in
        at all and had to find the emailed link. That could not survive the
        produce market: a customer buying carrots has no club membership to pay
        for, so the block moved from the account to the membership.

        They now sign in perfectly well and land on the payment screen. What
        they still cannot do is *use the club* — see
        `test_a_registered_member_holds_no_permissions`.
        """
        result = services.register_member(**self.submission())
    ...
    def test_a_registered_member_holds_no_permissions(self):
        """The half of the old assertion that survives, moved to where it is
        now enforced. Signing in and being able to use the club are different
        questions, and only the second is gated on payment."""
        result = services.register_member(**self.submission())
        loaded = User.objects.with_platform_roles().get(pk=result.user.pk)
    ...
    def test_the_nickname_keeps_the_capitalisation_the_member_chose(self):
        ...
        self.assertEqual(result.user.club_nickname, 'GrowerOne')
...
class NicknameTests(RegistrationTestCase):
    ...
    *and* mobile number. All three are duplicate keys and all three are checked
    ...
    def test_an_erased_member_does_not_hold_their_nickname(self):
        ...
        self.assertTrue(result.created)
        self.assertEqual(result.user.club_nickname, 'Grower')
