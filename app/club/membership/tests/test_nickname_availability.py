...
from app.club.membership.throttles import NicknameAvailabilityThrottle
...
class NicknameAvailabilityEndpointTests(TestCase):
    ...
    def member(self, nickname, email='holder@example.com'):
        return make_member(email, nickname, account=make_account(email, status=UserStatus.ACTIVE))
...
class TakenTests(NicknameAvailabilityEndpointTests):
    ...
    def test_a_suspended_member_keeps_their_nickname(self):
        make_member('suspended@example.com', 'Grower', account=make_account('suspended@example.com', status=UserStatus.SUSPENDED))
...
class ServiceTests(TestCase):
    ...
    def test_a_taken_nickname_is_not(self):
        make_member('holder@example.com', 'Grower', account=make_account('holder@example.com', status=UserStatus.ACTIVE))
    ...
    def test_an_erased_member_holds_no_nickname(self):
        ...
        user = make_member('holder@example.com', 'Grower', account=make_account('holder@example.com', status=UserStatus.ACTIVE))
        user.soft_delete()
