...
business; it is now ``app.core.payments``, which owns the price, the Payfast mandate
...
from app.core.documents.models import DocumentConsent
from app.core.payments import services as payment_services
...
#: ``app.core.payments`` that moves an account off it, and it does so through
#: ``User.activate`` rather than by writing this column.
#: Where registration leaves the **membership**. The account itself is Active
#: from the moment it exists: it is an identity, and there is nothing to verify
#: before somebody may hold one. What is outstanding is the club subscription,
#: and that is a fact about belonging rather than about signing in.
#:
#: The visible consequence is the reason for the change. A member registered and
#: unpaid now signs in and is sent to the payment screen, where before they were
#: refused at the door and had to find the emailed link. C27, and
#: `design/verticals.md` section 5.
REGISTERED_MEMBERSHIP_STATUS = MembershipStatus.PENDING_PAYMENT
...
#: Registration grants no role, because there is no role column -- C28. What a
#: completed registration creates is a `ClubMembership`, and what that grants is
#: resolved from the row rather than stamped onto the account.
...
def _resolve_consents(submitted):
    ...
    """
    # The club's storefront, named rather than resolved from a request. This
    # service creates a `ClubMembership`; joining the club is club-scoped by
    # definition, and reading the storefront off the host here would make it
    # possible to sign up for the club through the market's domain.
    in_force = {
        revision.document.slug: revision
        for revision in document_services.current_revisions(Storefront.CLUB)
    }
    ...
    return document_services.resolve_submitted(submitted, Storefront.CLUB)
...
def nickname_is_available(nickname):
    ...
    return not ClubMembership.objects.nickname_is_taken(nickname)
...
def register_member(
    *,
    ...
    # `status` is not set here and the column default -- Active -- is what
    # stands. The nickname is not set here either: it belongs to the membership
    # written below, not to the person. Both moved with C27.
    ...
    # The membership, in the same transaction as the account. Two rows now
    # where there was one, and the split is the whole of C27: the account says
    # who this is and may sign in, and this says they have joined the club and
    # have not paid for it yet.
    #
    ...
    # In the same transaction as the member, their membership and their
    # agreements, and that is the point. A membership at Pending payment with no
    # subscription has no way to leave that status, and a subscription with no
    # member is a mandate against nobody -- so either all four exist or none of
    # them do.
