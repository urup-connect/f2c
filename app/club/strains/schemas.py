...
class CultivatorOut(Schema):
    ...
    of ``roles-and-permissions.md`` makes a public name the rule for every
    payload, and a picker is where that rule is most easily broken -- an email
    address would make an autocomplete marginally better and would put a
    grower's address in a browser on an administrator's desk.

    **The picker offers producers, not people.** A listing belongs to the farm,
    and a picker offering three appointed staff of one farm as three cultivators
    was the shape this had while ``cultivator`` pointed at a user. The field is
    still called ``display_name`` because that is what the screen renders it
    into; what fills it is ``Producer.pseudonym``.

    No status, no counts. The endpoint has already filtered to the producers on
    the platform; anything more would be a directory, which this is not.
    """
...
class StrainRowOut(Schema):
    ...
    def resolve_reserved_to(obj):
        return obj.exclusive_to.pseudonym if obj.exclusive_to_id else None
...
class ListingRowOut(Schema):
    ...
    def resolve_cultivator(obj):
        """The producer's public name, for the reason ``reserved_to`` gives."""
