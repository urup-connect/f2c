...
def make_cultivator(email):
    """The farm a listing belongs to.

    **A `Producer`, not a person.** `CultivatorStrainListing.cultivator` and
    `Strain.exclusive_to` both point at the organisation now: a listing and an
    exclusivity arrangement are the farm's, and must survive the departure of
    whichever grower set them up.

    Keeps the name and the email-shaped argument so the call sites read
    unchanged — the address becomes the trading name, which is all these tests
    need it for.
    """
    return make_producer(email.split('@')[0])
