...
class ListTests(ApiTestCase):
    ...
    def test_reserved_to_is_a_display_name(self):
        ...
        self.assertEqual(row['reserved_to'], self.cultivator.pseudonym)
        self.assertNotIn(self.grower.email, json.dumps(row))
...
class ReadTests(ApiTestCase):
    ...
    def test_exclusive_to_is_sent_as_an_id_and_as_a_name(self):
        ...
        self.assertEqual(payload['reserved_to'], self.cultivator.pseudonym)
...
class ListingsInDetailTests(ApiTestCase):
    ...
    def test_an_offer_names_the_cultivator_by_display_name(self):
        ...
        self.assertEqual(entry['cultivator'], self.cultivator.pseudonym)
        self.assertNotIn(self.grower.email, json.dumps(entry))
...
class CultivatorPickerTests(ApiTestCase):
    ...
    def test_it_carries_no_email_address(self):
        ...
        self.assertNotIn(self.grower.email, body)
        self.assertNotIn('email', body)

    def test_every_producer_is_offered(self):
        """**A rule lost in the producer generalisation, recorded not hidden.**

        This asserted that a grower who had left was not offered, by erasing
        their account. A `Producer` is an organisation: it is not erased, and it
        has no departure state — so every producer is offered and the old rule
        has nothing to hang on. `strains.services._validated_exclusive_to` says
        the same, and `todo.md` carries the gap.
        """
        self.grower.soft_delete()

        self.assertEqual(len(self.cultivators()), 1)
