...
from app.documents.models import ClubDocument, DocumentVersion


class Command(BaseCommand):
    help = 'Upload a club document revision and, unless told otherwise, publish it.'

    def add_arguments(self, parser):
        parser.add_argument(
            'document', help='Slug of an existing club document, e.g. constitution'
        )
    ...
    def handle(self, *args, **options):
        try:
            document = ClubDocument.objects.get(slug=options['document'])
        except ClubDocument.DoesNotExist:
            known = ', '.join(ClubDocument.objects.values_list('slug', flat=True))
            raise CommandError(
                f'No club document with slug "{options["document"]}". Known: {known}'
            )
