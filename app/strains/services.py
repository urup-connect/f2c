...
def reservable_cultivators(user):
    ...
    **What this exposes.** Only an id and a ``display_name``, and that is not a
    ...
    Ordered by nickname, which is what ``display_name`` falls back to first --
    ...
    return (
        ...
        .filter(role=UserRole.CULTIVATOR, deleted_at__isnull=True)
        .order_by('nickname')
    )
...
def _validated_exclusive_to(account_id):
    ...
    if (
        account is None
        or account.role != UserRole.CULTIVATOR
        or account.deleted_at is not None
    ):
        raise ValidationError(
            'A strain can only be reserved to an active cultivator.',
