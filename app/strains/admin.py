...
def cultivator_choices(field):
    ...
    Erased accounts are excluded. ``soft_delete`` leaves the role in place
    deliberately -- it is a fact about the club's structure rather than about the
    person -- so filtering on the role alone would keep offering them.
    """
    return field.remote_field.model.objects.filter(
        role=UserRole.CULTIVATOR, deleted_at__isnull=True
    ).order_by('nickname')
...
class StrainAdmin(admin.ModelAdmin):
    ...
    @admin.display(description='Reserved to', ordering='exclusive_to__nickname')
...
class CultivatorStrainListingAdmin(admin.ModelAdmin):
    ...
    search_fields = ('strain__name', 'cultivator__nickname', 'short_description')
