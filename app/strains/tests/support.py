...
class CatalogueTestCase(TestCase):
    ...
    def setUp(self):
        self.admin = self.account(
            'admin@example.com', 'Curator', UserRole.ADMIN
        )
        self.cultivator = self.account(
            'grower@example.com', 'Kloof', UserRole.CULTIVATOR
        )
        self.member = self.account(
            'member@example.com', 'Thabo', UserRole.MEMBER
        )

    def account(self, email, nickname, role):
        """An Active account holding ``role``.

        Active deliberately: ``permissions_for`` empties the set for an account
        that cannot sign in, so a Pending administrator would be refused for the
        wrong reason and a test asserting a 403 would pass without testing
