...
class PermissionTests(CatalogueTestCase):
    ...
    def test_a_cultivator_may_not(self):
        ...
        with self.assertRaises(PermissionDenied):
            services.catalogue(self.cultivator)
...
class ExclusivityTests(ServiceWriteTestCase):
    ...
    def test_a_strain_may_not_be_reserved_to_a_departed_cultivator(self):
        """`soft_delete` leaves the role in place on purpose -- it is a fact
        about the club's structure rather than about the person -- so filtering
        on the role alone would still admit somebody who has left."""
        self.cultivator.deleted_at = timezone.now()
        self.cultivator.save()

        with self.assertRaises(ValidationError):
            self.create(exclusive_to=self.cultivator)
...
class CatalogueReadTests(ServiceWriteTestCase):
    ...
    def test_the_counts_do_not_multiply_each_other(self):
        ...
        for index in range(3):
            self.listing(
                self.kush,
                cultivator=self.account(
                    f'grower{index}@example.com', f'Farm{index}', UserRole.CULTIVATOR
                ),
