...
class Strain(models.Model):
    ...
    exclusive_to = models.ForeignKey(
        ...
        help_text='Leave blank for a strain any cultivator may offer. Set it to '
                  'reserve the strain to one cultivator -- their own genetics, '
...
class CultivatorStrainListing(models.Model):
    ...
    **``cultivator`` points at a user, and should point at a farm.** Block 2
    ...
    def __str__(self):
        # `display_name`, never a name or an email address. Section 6.6 of
        # `roles-and-permissions.md` makes that a property of every payload, and
        # a `__str__` reaches admin log entries and error reports. The previous
        # version rendered `cultivator.id`, which is safe but unreadable.
        return f'{self.cultivator.display_name} - {self.strain.name}'
