...
#: The statuses a payment may lift an account out of. ``PENDING_PAYMENT`` is
#: where registration leaves a member; ``SUSPENDED`` is where ``lapse_overdue``
#: puts one whose subscription stopped paying, and paying again undoes it.
#:
#: ``PENDING`` is deliberately absent: that is an account awaiting *verification*
#: by the club, and money does not settle that question. So is ``INACTIVE``,
#: which is where erasure lands -- ``User.activate`` refuses an erased account
#: anyway, and this says so a step earlier.
ACTIVATABLE_STATUSES = (UserStatus.PENDING_PAYMENT, UserStatus.SUSPENDED)
...
class NotificationUnconfirmed(Exception):
    ...
    Separate from :class:`~app.payments.gateway.NotificationRejected` because it
...
def _activate_account(user):
    """Let a paid-up member sign in.

    Three outcomes, and keeping them apart is what makes the log usable.

    An account at ``PENDING_PAYMENT`` or ``SUSPENDED`` is activated. An account
    already ``ACTIVE`` is left alone **quietly** -- that is every renewal, which
    is the majority of notifications this application will ever see, and warning
    about it would bury the one case that matters under a monthly flood.

    Anything else is recorded and warned about rather than refused. A payment
    against an account awaiting verification, or an erased one, is a thing that
    actually happened and must still be written down; refusing the notification
    would only make Payfast retry it forever and would lose the money's trail.
    The payment row is the record, and this is how a human finds out.
    """
    if user.status == UserStatus.ACTIVE:
        return False

    if user.status in ACTIVATABLE_STATUSES:
        user.activate()
        return True

    logger.warning(
        'payments: payment recorded against account %s at status %s; not '
...
def _apply_completion(subscription, posted, now):
    ...
    _activate_account(subscription.user)
...
def lapse_overdue(*, today=None):
    ...
    for subscription in (
        Subscription.objects.overdue(today).select_related('user').iterator()
    ):
        with transaction.atomic():
...
def _send_checkout_link(email, name, url, amount):
    ...
    body = render_to_string(
        ...
        {'name': name, 'url': url, 'amount': f'{amount:.2f}'},
    )
    EmailMessage(
        subject='Complete your Cultivators Collective membership payment',
        body=body,
        to=[email],
    ).send()
