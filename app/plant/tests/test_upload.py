...
class CommandTests(PlantTestCase):
    def upload(self, rows, *args, tmp=None, **kwargs):
        ...
        call_command(
            ...
            '--cultivator', 'grower@example.com', *args, **kwargs
        )
        return path
    ...
    def test_an_unknown_cultivator_is_refused(self):
        with self.assertRaises(CommandError):
            call_command(
                'upload_plants', 'x.xlsx', '--cultivator', 'nobody@example.com'
            )

    def test_an_erased_account_is_refused(self):
        """`soft_delete` keeps the role deliberately, so the role alone would
        still match one."""
        import tempfile
        from pathlib import Path
        ...
        self.assertIn('No account matches', str(refused.exception))
    ...
    def test_the_template_command_writes_a_workbook(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            call_command(
                'plant_template', 'grower@example.com',
