...
from app.plant.services import template_reference
from app.plant.spreadsheet import build_template
    ...
    def handle(self, *args, **options):
        ...
        if not reference:
            ...
            self.stderr.write(self.style.WARNING(
                f'{cultivator.display_name} has no listed strain offerings, so '
        ...
        destination = Path(
            options['output']
            or f'stock-template-{cultivator.display_name}.xlsx'.replace(' ', '-')
        )
