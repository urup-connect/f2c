...
class RenewalTests(ApplyMixin, PaymentsTestCase):
    ...
    def test_a_renewal_for_an_active_member_is_quiet(self):
        ...
        with self.assertNoLogs('app.core.payments.services', level='WARNING'):
            self.apply(notification(self.subscription, payment_id='PF-2'))
    ...
    def test_a_payment_against_an_account_awaiting_verification_is_flagged(self):
        ...
        self.member.club_membership.status = MembershipStatus.PENDING
...
class LapseTests(ApplyMixin, PaymentsTestCase):
    ...
    def test_lapsing_lapses_the_membership_and_leaves_the_account(self):
        """Reversible: paying again undoes it, and nothing personal is touched.

        **Two things changed with C27.** The membership goes to `LAPSED` rather
        than the account to `SUSPENDED` — its own status, so "stopped paying" is
        not recorded as the same thing as "the club blocked them" — and the
        account stays Active, because locking it would sign somebody out of the
        produce market over a club subscription.
        """
        self.overdue_by(1)
        services.lapse_overdue()
        _, member = self.reload()

        self.assertEqual(member.club_membership.status, MembershipStatus.LAPSED)
        self.assertTrue(member.is_active)
    ...
    def test_it_does_not_overwrite_a_suspension_staff_applied(self):
        """A membership suspended for conduct is not quietly relabelled 'did not
        pay'. Only an active one lapses."""
        self.member.refresh_from_db()
        self.member.club_membership.status = MembershipStatus.SUSPENDED
        self.member.club_membership.save(update_fields=['status'])
        self.overdue_by(1)
...
class OutstandingLinkTests(PaymentsTestCase):
    ...
    def test_it_always_leaves_by_the_club_server_as_the_club(self):
        """The subscription is the club's alone -- the market has none to bill.

        So this is named outright rather than resolved from a request. A produce
        customer must never receive a club membership demand, and the club's
        provider is the only one authorised to send as the club anyway.
        """
        mail.outbox.clear()
        ...
        self.assertEqual(mail.outbox[0].sent_using, 'club')
        self.assertEqual(mail.outbox[0].from_email, 'no-reply@club.example.co.za')
        self.assertIn('Cultivators Collective', mail.outbox[0].subject)
