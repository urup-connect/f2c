...
# ``frontend/club/lib/sa-mobile-number.ts``, ``frontend/club/lib/nickname.ts``,
# ``frontend/club/lib/person-name.ts`` and ``frontend/club/lib/email-address.ts``. These
...
def is_at_least(born, years, today=None):
    ...
    **Calendar arithmetic, not milliseconds.** Adding eighteen years to a
    ...
    part by part has none of that, and matches ``frontend/club/lib/age-gate.ts``
...
#: ``RESERVED_NICKNAMES`` in ``frontend/club/lib/nickname.ts``.
RESERVED_NICKNAMES = frozenset({
    ...
    'cultivators', 'f2c', 'signup', 'login', 'verify', 'api',
    'age-check',
})
...
def validate_nickname(value):
    ...
    question, and ``membership.ClubMembership.objects.nickname_is_taken``
    answers it -- on the membership since C27, not on the account.
    """
...
def validate_person_name(value):
    ...
    ``frontend/club/lib/person-name.ts`` holds the fuller rule, and is what a member
