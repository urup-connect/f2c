...
from app.core.accounts.schemas import UserOut
from app.core.common.schemas import MessageOut
...
async def login_start(request, payload: EmailIn):
    ...
    if credentials:
        options, challenge = wa.authentication_options(
            allow=credentials, storefront=storefront_for_request(request)
        )
    ...
    if user is not None:
        await otp_service.issue(user, storefront=storefront_for_request(request))
    return {'method': 'otp'}
...
async def me(request):
    ...
    Re-read through ``with_platform_roles()`` rather than returned straight from
    ``auser()``. ``UserOut`` serialises the club nickname and the membership
    status, both of which live on the reverse one-to-one, and touching an
    unloaded relation inside an async view raises ``SynchronousOnlyOperation``.
    The cost is one indexed primary-key lookup on a request that was going to
    make the lazy query anyway.
    """
...
async def passkey_register_options(request):
    ...
    """
    # Re-read with the membership attached: `display_name` prefers the club
    # nickname, which lives one table away since C27, and touching an unloaded
    # relation inside an async view raises `SynchronousOnlyOperation`. The third
    # place in this module that has to say so — see `UserManager.with_platform_roles`.
    user = await request.auser()
    user = await User.objects.with_platform_roles().aget(pk=user.pk)
