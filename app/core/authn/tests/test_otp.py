...
class IssueTests(TestCase):
    def setUp(self):
        self.user = make_member(
            'member@example.com', 'Bean', first_name='Craig'
        )
    ...
    async def test_the_code_is_emailed_to_the_member(self):
        ...
        # The greeting is the account's own first name, **not** the club
        # nickname. `get_short_name` used to return the nickname, which lives
        # one table away since C27 — and this path is `async`, where a lazy
        # relation is fatal. A produce-market customer has no nickname either.
        self.assertIn('Craig', mail.outbox[0].body)
