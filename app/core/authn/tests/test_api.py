...
from app.core.accounts.roles import MEMBER_PERMISSIONS
...
from app.core.authn.tests.test_otp import code_from_last_email
...
STOREFRONT_ROUTING = dict(
    ALLOWED_HOSTS=[
        'club.example.co.za',
        'market.example.co.za',
        'unmapped.example.co.za',
        'testserver',
    ],
    STOREFRONT_HOSTS={
        'club.example.co.za': 'club',
        'market.example.co.za': 'market',
    },
...
class ApiTestCase(TestCase):
    ...
    def make_member(self, email='member@example.com', **overrides):
        """An account, and a club membership when a nickname is asked for.

        `nickname` is not a `User` field since C27, so it is taken out of the
        account's keywords and used to build the membership instead — which is
        what these tests mean by it.
        """
        nickname = overrides.pop('nickname', None)
        fields = {'status': UserStatus.ACTIVE}
        fields.update(overrides)
        user = User.objects.create_user(email=email, **fields)
        if nickname is not None:
            # **Active**, not the column default of `pending`. A pending
            # membership grants nothing — correctly — and a fixture that left it
            # there would make every permission assertion below pass by being
            # empty rather than by being right.
            ClubMembership.objects.create(
                user=user, nickname=nickname, status=MembershipStatus.ACTIVE
            )
        return user
...
class OtpStorefrontRoutingTests(ApiTestCase):
    """Which server the sign-in code leaves by, and whose name is on it.

    The club and the store are separate businesses with separate mailbox
    providers, so a code requested at the store must leave the store's server
    signed with the store's name. Getting this wrong sends successfully -- the
    member receives a club-branded code from a provider that has no relationship
    with them, which is indistinguishable from a phishing attempt and is exactly
    the thing a one-time code has to be trusted against.

    The storefront comes from the **host**, not from what the member belongs to.
    An address with no account at all has to be answerable, and a member of both
    storefronts signing in at the store should be answered by the store.
    """
    ...
    def test_a_code_requested_at_the_club_leaves_by_the_club_server(self):
        ...
        self.start('member@example.com', 'club.example.co.za')

        message = mail.outbox[-1]
        self.assertEqual(message.sent_using, 'club')
        self.assertEqual(message.from_email, 'no-reply@club.example.co.za')
        self.assertIn('Cultivators Collective', message.subject)

    @override_settings(**STOREFRONT_ROUTING)
    def test_a_code_requested_at_the_store_leaves_by_the_store_server(self):
        self.make_member()

        self.start('member@example.com', 'market.example.co.za')

        message = mail.outbox[-1]
        self.assertEqual(message.sent_using, 'market')
        self.assertEqual(message.from_email, 'no-reply@market.example.co.za')
        self.assertIn(Storefront.MARKET.label, message.subject)
        self.assertNotIn('Cultivators Collective', message.subject)
        self.assertNotIn('Cultivators Collective', message.body)

    @override_settings(**STOREFRONT_ROUTING)
    def test_the_host_decides_and_not_the_membership(self):
        """A club member signing in at the store is answered by the store.

        The alternative -- routing on the member's own memberships -- cannot be
        asked of an address with no account, and would brand the store's mail
        as the club's for anybody who happens to hold both.
        """
        self.make_member()

        self.start('member@example.com', 'market.example.co.za')

        self.assertEqual(mail.outbox[-1].sent_using, 'market')

    @override_settings(**STOREFRONT_ROUTING)
    def test_an_unmapped_host_falls_back_to_the_default_storefront(self):
        """A preview deployment or a health check must not break sign-in."""
        self.make_member()

        self.start('member@example.com', 'unmapped.example.co.za')

        self.assertEqual(mail.outbox[-1].sent_using, 'club')
...
class OtpVerifyTests(ApiTestCase):
    ...
    def test_the_role_and_what_it_permits_are_returned(self):
        ...
        # Derived, and for routing only — there is no role column. C28.
        self.assertEqual(payload['role'], 'member')
        self.assertEqual(
            payload['permissions'], sorted(MEMBER_PERMISSIONS)
        )
...
class PasskeyEnrolmentTests(ApiTestCase):
    ...
    def test_a_verified_credential_is_stored(self):
        ...
        with patch(
            'app.core.authn.webauthn.verify_registration', return_value=self.verified_registration()
        ):
    ...
    def test_a_credential_that_fails_verification_is_not_stored(self):
        ...
        with patch(
            'app.core.authn.webauthn.verify_registration',
...
class PasskeyLoginTests(ApiTestCase):
    ...
    def test_a_full_round_trip_opens_a_session(self):
        ...
        with patch(
            'app.core.authn.webauthn.verify_authentication',
