...
def rp_id(storefront=None):
    """The relying-party ID for one storefront.

    **A passkey is scoped to a registrable domain and cannot cross one.** The
    club and the produce market sit on separate domains, so a credential
    enrolled on one is not presentable on the other, and the RP ID is a property
    of the storefront rather than of the deployment. A member of both enrols
    twice; the emailed code works anywhere. ``design/verticals.md`` section 8
    records the consequence and what a single sign-on would cost instead.

    ``storefront`` is optional so that a caller with no request -- a shell, a
    test -- gets the deployment's default rather than having to invent one. That
    is also what keeps a single-storefront deployment configured exactly as it
    was: leave ``DJANGO_WEBAUTHN_RP_IDS`` unset and every storefront resolves to
    ``DJANGO_WEBAUTHN_RP_ID``.
    """
    value = (settings.WEBAUTHN_RP_IDS or {}).get(storefront) if storefront else None
    value = value or settings.WEBAUTHN_RP_ID
    if not value:
        raise ImproperlyConfigured(
            'DJANGO_WEBAUTHN_RP_ID is not set. It must be the registrable '
            'domain the frontend is served from, e.g. f2c.co.za. Set '
            'DJANGO_WEBAUTHN_RP_IDS as well once a second storefront is live.'
        )
    return value
...
def registration_options(
    *, user_handle, user_name, user_display_name, exclude, storefront=None
):
...
def authentication_options(*, allow, storefront=None):
...
def verify_registration(*, credential, challenge, storefront=None):
    """Verify a ``create()`` response. Raises on anything unexpected."""
    return verify_registration_response(
        credential=credential,
        expected_challenge=challenge,
        expected_rp_id=rp_id(storefront),
        expected_origin=origins(),
    )


def verify_authentication(
    *, credential, challenge, public_key, sign_count, storefront=None
):
