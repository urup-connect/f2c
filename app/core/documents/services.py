...
def published_documents(storefront):
    """Every document an unauthenticated caller may read, with its live revision.

    The store's legal pages: privacy notice, terms, data policy — and the club's
    rules, which are readable before an account exists for the same reason
    ``current_revisions`` is unauthenticated.

    **Unlike ``current_revisions``, a document with no published revision is
    skipped rather than raising.** The difference is what the caller does with
    the answer: sign-up must refuse the form outright if a required document is
    missing, because a member cannot agree to something that is not there. A
    legal-pages index is a list, and a list with one entry missing is still a
    usable page while somebody publishes the missing revision.
    """
    documents = list(
        Document.objects.readable_without_an_account(storefront)
    )
    latest = _latest_by_document(documents)
    return [latest[d.id] for d in documents if d.id in latest]


def current_revisions(storefront, *, required_only=True):
    """The revision in force for every document agreed to at registration.

    :raises DocumentsNotReady: if any document in scope has none.
    """
    documents = list(
        Document.objects.agreed_at_registration(storefront)
        if required_only
        else Document.objects.for_storefront(storefront)
    )
    latest = _latest_by_document(documents)
...
def outstanding_for(user, storefront, *, required_only=True):
...
def resolve_submitted(submitted, storefront, *, required_only=True):
    ...
    """
    revisions = current_revisions(storefront, required_only=required_only)
