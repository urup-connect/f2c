...
from app.core.documents.admin import (
    DocumentAdmin,
    DocumentConsentAdmin,
    DocumentVersionAdmin,
)
from app.core.documents.models import Document, DocumentConsent, DocumentVersion
...
class DocumentAdminTests(AdminTestCase):
    ...
    def test_the_change_form_loads_with_the_revision_inline(self):
        version = self.published(content=b'rules')
        response = self.client.get(
            reverse('admin:documents_document_change', args=[version.document.pk])
        )
        self.assertEqual(response.status_code, 200)
