...
class CurrentRevisionsTests(DocumentsTestCase):
    ...
    def test_every_document_at_its_live_revision_in_form_order(self):
        self.publish_all()
        revisions = services.current_revisions(Storefront.CLUB)
        self.assertEqual(
    ...
    def test_a_missing_revision_refuses_the_whole_list(self):
        ...
        with self.assertRaises(services.DocumentsNotReady) as caught:
            services.current_revisions(Storefront.CLUB)
        self.assertEqual(
    ...
    def test_a_draft_does_not_count_as_published(self):
        ...
        revisions = {r.document.slug: r for r in services.current_revisions(Storefront.CLUB)}
    ...
    def test_a_retired_document_drops_out_of_signup(self):
        ...
        # Retiring is `retired_at`, not the agreement enum: setting the enum
        # to `none` would retire it *and* publish it as a public page.
        retired.retired_at = timezone.now()
        retired.save()
        self.assertEqual(
            [revision.document.slug for revision in services.current_revisions(Storefront.CLUB)],
...
class OutstandingTests(DocumentsTestCase):
    ...
    def test_a_new_member_owes_the_live_revision(self):
        ...
        self.assertEqual(services.outstanding_for(self.member_account, Storefront.CLUB), [version])

    def test_a_member_who_agreed_owes_nothing(self):
        version = self.published(document=self.rules, content=b'v1')
        services.record_consents(self.member_account, [version])
        self.assertEqual(services.outstanding_for(self.member_account, Storefront.CLUB), [])
    ...
    def test_a_material_change_asks_again(self):
        ...
        self.assertEqual(services.outstanding_for(self.member_account, Storefront.CLUB), [second])
...
class ResolveSubmittedTests(DocumentsTestCase):
    ...
    def test_a_matching_submission_resolves_to_the_live_revisions(self):
        resolved = services.resolve_submitted(self.submission(), Storefront.CLUB)
        self.assertEqual(
    ...
    def test_a_stale_version_is_refused_rather_than_upgraded(self):
        ...
        with self.assertRaises(ValidationError) as caught:
            services.resolve_submitted(self.submission(**{'club-rules': '0'}), Storefront.CLUB)
    ...
    def test_a_missing_document_is_refused(self):
        ...
        with self.assertRaises(ValidationError) as caught:
            services.resolve_submitted(submission, Storefront.CLUB)
