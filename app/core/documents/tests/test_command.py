"""Tests for ``manage.py publish_document``.
...
class PublishCommandTests(DocumentsTestCase):
    ...
    def run_command(self, *args, **options):
        """`--storefront` defaults to the club here, and is required by the
        command itself: a slug is only unique within a storefront, so
        "publish terms v2" is an ambiguous instruction."""
        out = StringIO()
        options.setdefault('storefront', Storefront.CLUB)
        call_command('publish_document', *args, stdout=out, **options)
        return out.getvalue()
