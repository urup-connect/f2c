...
def document_upload_to(instance, filename):
    """``documents/<storefront>/<document>/<version>/<file name>``.

    The storefront leads, and it is not decoration: nothing is shared between
    the two, so the club and the market may each have a ``terms`` at ``v1``.
    Without this segment those are one key, and the second upload would
    overwrite a file somebody has already agreed to -- which is the one thing
    the path below is built to prevent.
