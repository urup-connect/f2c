...
from app.core.accounts.forms import UserChangeForm, UserCreationForm
...
class ChangeFormTests(TestCase):
    ...
    def test_leaving_the_field_blank_keeps_the_number_on_file(self):
        ...
        user = self.submit(first_name='Bean')

        self.assertEqual(user.first_name, 'Bean')
        self.assertEqual(user.id_number, VALID_SA_ID)
...
class ContactClashTests(TestCase):
    ...
    def setUp(self):
        self.held = User.objects.create_user(
            email='held@example.com', mobile='0821234567'
        )
    ...
    def test_a_member_may_keep_their_own_number(self):
        ...
        form = UserChangeForm(
            data=self.data(email='held@example.com', password=self.held.password,
                           mobile='0821234567', first_name='Craig'),
            instance=self.held,
        )
    ...
    # ------------------------------------------------------------------
    # Nickname — moved out with the field
    # ------------------------------------------------------------------
    # Six tests stood here, holding the nickname clash to a sentence beside the
    # field rather than a 500. They tested `ContactClashMixin.clean_nickname` on
    # the member form, and **the member form has no nickname field**: it moved to
    # `ClubMembership` with C27, and so did its uniqueness index.
    #
    # The rule itself is not untested — `test_uniqueness_keys` holds it against
    # the constraint, which is where it is actually enforced. What is untested is
    # the *form-level courtesy*, and it will come back with the `ClubMembership`
    # admin that `todo.md` already carries as outstanding. Recorded here so that
    # admin arrives with the tests it needs rather than without them.
