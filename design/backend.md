...
| Application settings for the database | `DJANGO_DB_HOST`, `_NAME`, `_USER`, `_PASSWORD`, plus one of `DJANGO_DB_SSL_DISABLED` or `DJANGO_DB_SSL_CA` — a deployed connection naming neither is refused, and naming both is refused rather than resolved. **QA and production set `DJANGO_DB_SSL_DISABLED=true`**: the connection is still TLS under mysqlclient's `PREFERRED`, but the certificate and hostname are not checked, and the firewall's Azure-services-plus-named-IPs rule carries the trust instead — C37. Section 8.0 |
| Static file handling | `STATIC_ROOT` plus WhiteNoise or a CDN. Still open |
| A real email provider | **Mostly done, and this row used to be wrong.** The console backend survives only under `DEBUG`, and `_mailer` refuses a deployed environment naming no host, so nothing could ever have shipped silently printing to a terminal. A cPanel provider is configured for both storefronts on 587 with STARTTLS and **both mailboxes authenticate** — the market's timed out during AUTH until 2 September 2026 and now succeeds, with no repository change. Left: neither QA nor production carries the values. P1 |
...
| 3 | Codes are printed to the console. No email provider is configured, so no member can sign in on a deployed environment. | Partly closed, and stated wrongly — the console backend survives only under `DEBUG` and `_mailer` refuses a deployed environment naming no host. A provider is configured for both storefronts and **both mailboxes authenticate** as of 2 September 2026; QA and production carry none of the values. P1 |
