...
- ~~**Nothing schedules `lapse_memberships`.**~~ **Built.** `app/core/payments/tasks.py` runs it
  nightly on Celery beat at 02:30 UTC — 04:30 SAST — and every run leaves a `scheduling.ScheduledRun`
  row in the admin, so "when was this member switched off, and by what?" is answerable by whoever
  takes the call. The home is **not** the timer-triggered Function App this line used to specify, nor
  the Container Apps Job that briefly replaced it: both put the schedule in platform configuration
  rather than in a reviewed commit, both left a failed run visible only in that platform's own logs,
  and neither could be exercised locally or in CI. No protected endpoint was needed. The management
  command survives as the same job by hand, with the `--dry-run` that answers "who would this switch
  off?". See `f2c/queue.py` and `design/deploy.md` 5.2. Risk 2 closed.
...
| 2 | Nothing runs `lapse_memberships` | An unpaid or cancelled membership keeps its access indefinitely | **Closed.** Celery beat runs it nightly at 02:30 UTC — 04:30 SAST — through `app/core/payments/tasks.py`, and each run is recorded in `scheduling.ScheduledRun`. What remains is provisioning: the worker and beat Container Apps, `design/deploy.md` 5.2. Until those exist in a given environment, the risk is open *there* — and `f2c/queue.py` refuses to start a deployed environment with no broker, so it cannot be open silently |
| 3 | No email provider on the deployed environments | The emailed payment link — the entire duplicate-registration fallback — reaches nobody there | **Partly closed, and this row used to be stated wrongly.** The console backend survives only under `DEBUG` and `_mailer` refuses a deployed environment naming no host, so nothing was ever silently printing to a terminal in QA. A cPanel provider is configured for both storefronts on 587 with STARTTLS and **both mailboxes authenticate** as of 2 September 2026, the market's having timed out during AUTH until then; neither QA nor production carries the values. Shared with sign-in codes — P1 |
