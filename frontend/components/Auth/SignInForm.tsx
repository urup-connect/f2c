...
const QUIET =
  'font-sans text-sm text-primary underline underline-offset-4 hover:text-forest-green-deep focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-forest-green disabled:no-underline disabled:opacity-60'
 ...
 *
 * **A failed passkey is not a dead end.** The member is told what happened and offered a code, and
 * the offer changes its wording so it reads as a fallback rather than as the same button again.
 * The alternative — silently swapping the form out — leaves somebody who cancelled by accident
 * looking at a screen they did not ask for.
 *
 ...
 * conditional: *if that address belongs to a member*. Copy that said "we have sent you a code"
 * would give away in words what the API is careful not to give away in bytes.
 *
 * The steps are two returns from one component rather than two components, because they share the
 * address, the busy flag and the feedback, and threading five pieces of state through props would
 * be more code holding the same thing together.
 */
export const SignInForm = () => {
  ...
  const finish = useCallback(
    (role: Parameters<typeof destinationAfterSignIn>[1]) => {
      /*
       * The refresh is not optional and has to come first: every club screen is a Server
       * Component that reads the session when it renders, and the router would otherwise serve
       * one from a cache built before the cookie existed.
       */
      router.refresh()
      router.push(destinationAfterSignIn(searchParams.get('next'), role))
    },
    [router, searchParams],
  )
    ...
    try {
      ...
      if (start.method === 'passkey' && start.options) {
        if (!browserSupportsWebAuthn()) {
          // Django prepared a challenge this browser cannot answer, and so sent no
          // code. Ask for one now rather than leaving the member stuck.
        ...
        try {
          ...
          const user = await loginWithPasskey(email, credential)
          finish(user.role)
          return
        } catch (caught) {
  ...
  const handleCodeSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
     ...
     * re-enable the button underneath a member who has already been signed in.
     */
  }
  ...
  if (step === 'code') {
    return (
      <form onSubmit={handleCodeSubmit} className="flex flex-col gap-4">
        ...
        <SignInFeedback problem={problem} notice={notice} />
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          >
            {cooldown > 0
              ? `${SIGN_IN.resendWaitingPrefix} ${cooldown}s`
              : SIGN_IN.resend}
          </button>
