 ...
 *
 * **A copy of `frontend/club/lib/sa-mobile-number.ts`, unchanged.** The rule is a platform rule and
 * belongs in `packages/` — see the same note on `person-name.ts`. One line of its reasoning reads
 * differently here and is worth saying out loud: the club calls this a contact number rather than a
 * credential, and in the store it is the number a driver rings from the gate. That makes it more
 * useful, not more required; it stays optional, because a wrong number is worse than none.
 */
