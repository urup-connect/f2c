 ...
 *
 * **A copy of `frontend/club/lib/email-address.ts`, unchanged.** The rule is a platform rule and
 * belongs in `packages/` — see the same note on `person-name.ts`. It matters more here than in the
 * club: the address is the sign-in identifier and the only thing a store customer is asked for, so
 * a typo in it is the whole of a lost account.
 */
