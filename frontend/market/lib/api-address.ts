 ...
 * into the browser bundle by `next build`, which made every image specific to one environment. A
 * build could not be promoted from QA to Production; it had to be rebuilt, and a promoted artefact
 * would have carried the wrong address while looking correct. That is `design/todo.md` Block 0 P6.
 *
 * The fix is not to drop the `NEXT_PUBLIC_` prefix. A client component has no `process.env` at
 * runtime, and the store has five of them — the sign-in and sign-up forms, the passkey card, the
 * account details form, the sign-out button. So the value is read here, on the server, once per
 * request, and written into the document as a `<meta>` tag that `lib/api.ts` reads in the browser.
 * The container is told the address; the bundle no longer knows it.
 *
 ...
 *
 * The club carries the same module for the same reason. It is one of the seams `packages/` will
 * eventually cover — see `frontend/README.md`.
 */
 ...
 * browser uses for this application** — `backend.f2c.co.za` with `f2c.co.za`, never
 * `backend.f2c-cannabis.co.za`. Otherwise the request is cross-site and the `SameSite=Lax` session
...
export const readPublicApiBaseUrl = (env: EnvRecord): string => {
  ...
  if (!value) {
    throw misconfigured(
      ...
      'not set. It is the API address the shopper’s browser uses, and it is read at request ' +
