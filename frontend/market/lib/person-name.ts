 ...
 *
 * **A copy of `frontend/club/lib/person-name.ts`, unchanged.** The rule is a platform rule — it is
 * what `accounts.profile` enforces, and a person's name does not become a different thing because
 * they are buying carrots — so this is one of the first files that should move into `packages/`.
 * It is duplicated rather than imported for the reason `frontend/README.md` records: the seams are
 * drawn once, against a second consumer, and drawing them file by file as the store is written
 * would mean drawing them from one side. The tests are duplicated with it, so a divergence shows up
 * as a failure here rather than as a refusal only the store makes.
 */
