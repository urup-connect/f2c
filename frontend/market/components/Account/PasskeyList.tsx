 ...
 *
 * Presentational: it fetches nothing and owns no state, so the list shown is whatever the card decided
 * it is. That is what makes "removing this one" and "a request is in flight" separately testable —
 * they are two props here and would be one boolean inside the component that fetches.
 *
 * Every row names the passkey in its own button. A screen full of buttons all called "Remove" is
 * unusable from a screen reader's list of controls, and the visible label stays short because the name
 * is added through `aria-label`.
 */
        ...
        >
          <div className="flex flex-col gap-1">
            <p className="font-sans text-base font-medium text-foreground">
              {passkey.name}
              {passkey.backed_up ? (
                <span className="ml-2 rounded-pill bg-leaf-pale px-2 py-0.5 font-sans text-xs uppercase tracking-label text-leaf">
                  {PASSKEYS_CARD.synced}
                </span>
              ) : null}
            </p>
            <p className="font-sans text-sm text-muted-foreground">{passkeyTimeline(passkey)}</p>
          </div>
          ...
          <button
            ...
            className="inline-flex h-10 items-center justify-center rounded-pill border-2 border-primary bg-transparent px-5 font-sans text-sm font-medium text-primary transition-colors hover:bg-leaf-pale focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-leaf disabled:opacity-60"
          >
