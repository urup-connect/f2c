 ...
 *
 * Derived from the heading rather than counted or generated: these render on the server and a
 * generated id would be one more thing that could differ between two renders of the same page for no
 * benefit. Two cards sharing a heading on one screen would collide, and that is a content mistake
 ...
 * reason this is a component rather than a `div` with a class: it is easy to leave out and impossible
 * to notice with a mouse.
 *
 ...
 */
export const Card = ({ heading, standfirst, note, children }: CardProps) => {
  const id = cardHeadingId(heading)

  return (
    <section aria-labelledby={id} className="rounded-card bg-surface p-6 shadow-sm sm:p-8">
      <h2 id={id} className="font-display text-2xl tracking-display text-leaf">
        {heading}
      </h2>

      {standfirst ? (
        <p className="mt-2 font-sans text-sm leading-relaxed text-muted-foreground">{standfirst}</p>
      ) : null}
