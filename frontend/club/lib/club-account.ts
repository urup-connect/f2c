...
/**
 * How the membership stands.
 *
 * **Reads `membership_status`, not `status`.** This card was always about the club and used to read
 * the account column because there was only one — the split gave the question its own field, and
 * pointing this at the account status now would report "Active" at a member who has not paid. C27.
 *
 * `null` — an account that never joined — is mapped to a written-out `none` rather than left to
 * render blank. The club gate turns that account away before this card draws, so it is unreachable;
 * a card that renders nothing for an unexpected value is a card that looks broken at the exact
...
export const membershipStanding = (
  ...
  return {
    label: MEMBERSHIP_CARD.statusLabels[key],
    note: MEMBERSHIP_CARD.statusNotes[key],
  }
}
