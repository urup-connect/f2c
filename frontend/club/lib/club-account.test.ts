...
const MEMBER: User = {
  ...
  status: 'active',
  /*
   * Both statuses, because the split gave the club membership its own field: `status` says whether
   * this identity may sign in, and this says where the membership stands. A paid-up member has both.
   * See `membershipStanding`, which is deliberately pointed at this one.
   */
...
describe('membershipStanding', () => {
  ...
  /*
   * `inactive` is gone and `lapsed` and `none` are new: this card reads the
   * **membership** status since C27, and erasure is a fact about the account.
   * `none` is an account that never joined — unreachable behind the club gate,
   * and written out so the card never renders blank.
   */
  test.each(['pending', 'pending_payment', 'suspended', 'lapsed', 'sharing', null] as const)(
