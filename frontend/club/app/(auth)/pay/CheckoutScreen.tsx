...
import { fetchCheckout, fetchMyCheckout } from '@/lib/checkout-api'
 ...
 */
export const CheckoutScreen = async ({
  token,
  allowSession = false,
}: {
  token: string | null
  /**
   * Whether to fall back to the signed-in member's own checkout when there is no token.
   *
   * Set by `/pay`, which is where the club's pay-now redirect lands. **Not** set by `/pay/[token]`:
   * that route is reached from an emailed link, and a hand-edited or expired token there must
   * answer "unavailable" rather than quietly charging whoever happens to be signed in on that
   * browser for a different membership.
   */
  allowSession?: boolean
}) => {
  /*
   * No token and no session fallback is answered as unavailable rather than as a fault. It means
   * the cookie expired, was never set, or the link was hand-edited — none of which is an error on
   * our side, and all of which are fixed by a fresh link.
   */
  if (!token && !allowSession) {
    return (
      <AuthCard>
  ...
  const outcome = token ? await fetchCheckout(token) : await fetchMyCheckout()
