...
import { requireClubMembership } from '@/lib/club-session'
 ...
 *
 * **Two gates, not one.** The session check answers whether this is anybody; `requireClubMembership`
 * then answers whether the club is open to them. They are separate because signing in and belonging
 * stopped being the same fact when the produce market arrived — an unpaid member now signs in and
 * is sent to the payment screen rather than refused at the door, and a market customer with no
 * membership is sent to the front door rather than shown a club they never joined. See
 * `lib/club-membership.ts` and C27.
 *
