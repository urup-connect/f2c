...
describe('MembershipSummary', () => {
  ...
  test('shows something rather than nothing for a role with no home', () => {
    ...
    // `sharing_member` is not a value `role` can take any more — it is derived
    // and only ever 'admin', 'cultivator' or 'member'. Cast, because what is
    // under test is that an unrecognised word renders rather than blanking.
    render(
      <MembershipSummary role={'sharing_member' as never} status="sharing" />,
    )
